...
public:
  NodoArbolBinario() : derecha(nullptr), izquierda(nullptr), dato(T()){};
...
template <class T> T ArbolBinario1Byte<T>::get(int i) {
  // Convertir el índice entero a un bitset<16> (para ser compatible con
  // buscarDato).
...
template <class T> T ArbolBinario1Byte<T>::buscarDato(bitset<16> posicion) {
...
template <class T> void ArbolBinario1Byte<T>::addArbol(T dato) {
  if (raiz == nullptr) {
