...
int Dijkstra ::encontrarMinimo(const int *distancia, const bool *visitado,
                               int n) {
...
int Dijkstra ::dijkstra(int s, int t) {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    if (u == -1)
      break; // Si no hay más nodos alcanzables
    visitado[u] = true;
...
int Dijkstra ::proximoNodo(int *predecesor, int s, int t) {
  ...
  // Retroceder en la lista de predecesores para encontrar el primer nodo del
  // camino
  int actual = t;
