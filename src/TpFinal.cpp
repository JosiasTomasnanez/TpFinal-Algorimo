...
public:
  NodoArbol() {
   derecha=nullptr; izquierda=nullptr;cerrada=false;
  };
...
void Router::separarPagina(Pagina pag) {
    int totalPaquetes = (pag.tamano + tamañoPaquete-1 ) / tamañoPaquete; // Calcula el número total de paquetes
...
void Terminal::escribirLog(Pagina* p){
        ...
        pot=pot>>8;
        pdt=pdt>>8;
        logfile<< "pagina: "<< p->id<<". origen: "<<por.to_ulong()<<"-"<<pot.to_ulong();
        logfile<<". destino: "<< pdr.to_ulong()<<"-"<<pdt.to_ulong();
        logfile<< ". router: "<<rip.to_ullong()<<". terminal: "<<id<<"."<< endl;
}
...
class AdministradorDelSistema{
 ...
 public:
  ...
  void initSim(int ciclosMax){for(int i=0;i<ciclosMax;i++){cout<<"ciclo: "<<i<< " Realizado"<<endl;ciclo(); }};
